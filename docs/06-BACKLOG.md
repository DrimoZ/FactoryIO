...
| FIO-066 | P1 | M | **Animer le bras.** Requalifié le 31/07/2026 après relecture de la géométrie : le ticket n'était **pas** bloqué par un travail de modélisation. Les cubes portent des coordonnées absolues, donc créer le bone du bras est une transformation JSON scriptable, pas une session Blockbench. Étude, options et plan en cinq étapes : [`11`](11-DESIGN-ANIMATION.md). | le bras suit le swing ; angle et pince couverts par JUnit |
| FIO-067b | P2 | M | **Réconcilier l'item transporté avec la pince** — aujourd'hui il suit une Bézier dont la « main » est une constante fictive. Animer le bras sans cela fait flotter l'item à côté de la pince, ce qui est pire que pas d'animation. Dépend de FIO-066. Voir [`11`](11-DESIGN-ANIMATION.md) §5. | l'item est dans la pince à toutes les portées |
