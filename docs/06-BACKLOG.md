...
| 🟡 FIO-092 | **P1** | M | **Bloc + BlockEntity + placement.** Les trois tiers existent, `connected` est résolu au placement et à `updateShape`, jamais au tick. Tier et sens sont des traits de la classe et non des propriétés d'état — sans quoi trois tiers × trois sens auraient multiplié par neuf les 32 variantes de blockstate. Reste à confirmer à l'œil. | les 8 variantes s'affichent correctement |
...
| FIO-097 | **P1** | M | **Interaction inserter ↔ convoyeur** (voie proche / voie lointaine). C'est le seuil de jouabilité : rien n'alimente encore un convoyeur automatiquement. Un `IItemHandler` est exposé sur toutes les faces (`BeltItemHandler`, cf. [`08`](08-DESIGN-BELTS.md) §7) — hoppers et inserters peuvent déjà y prendre et y déposer —, mais l'inserter ne sait pas viser une **voie**. | parité Factorio |
