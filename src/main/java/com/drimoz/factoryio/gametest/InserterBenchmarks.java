 ...
 * {@code InserterBlockEntity.tick} en boucle et chronomètrent : la mesure porte
...
public class InserterBenchmarks {
    ...
    public static void sleepingInsertersAreNearlyFree(GameTestHelper helper) {
        List<InserterBlockEntity> inserters = placeLoneInserters(helper);
    ...
    public static void activeInsertersStayWithinBudget(GameTestHelper helper) {
        List<InserterBlockEntity> inserters = placeWorkingChains(helper);
    ...
    private static double measure(List<InserterBlockEntity> inserters) {
    ...
    private static void tickAll(List<InserterBlockEntity> inserters) {
        for (InserterBlockEntity inserter : inserters) {
            InserterBlockEntity.tick(
     ...
     */
    private static List<InserterBlockEntity> placeLoneInserters(GameTestHelper helper) {
        List<InserterBlockEntity> inserters = new ArrayList<>();
     ...
     */
    private static List<InserterBlockEntity> placeWorkingChains(GameTestHelper helper) {
        List<InserterBlockEntity> inserters = new ArrayList<>();
        Inserter definition = definition("inserter");
        ...
        for (int y = 1; y < 8; y += 2) {
            for (int z = 1; z < 15; z += 2) {
                for (int x = 1; x + 2 < 16; x += 3) {
                    ...
                    InserterBlockEntity inserter = place(helper, definition, middle);
    ...
    private static InserterBlockEntity place(
        ...
        return (InserterBlockEntity) helper.getBlockEntity(pos);
    }
