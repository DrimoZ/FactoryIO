 ...
 */
public class FactoryIOSyncC2SInserterSetting {
	...
	public FactoryIOSyncC2SInserterSetting(BlockPos pos, Setting setting, int value) {
		this(pos, setting.ordinal(), value);
	}

	private FactoryIOSyncC2SInserterSetting(BlockPos pos, int setting, int value) {
		this.pos = pos;
		this.setting = setting;
		this.value = value;
	}

	public FactoryIOSyncC2SInserterSetting(FriendlyByteBuf buf) {
	...
	private void apply(FactoryIOInserterBlockEntity blockEntity, Setting target) {
		switch (target) {
			case FILTER_MODE -> {
