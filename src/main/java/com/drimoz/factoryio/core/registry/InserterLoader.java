...
public class InserterLoader {
    ...
    public static void setup() {
        ...
        InserterRegistry.getInstance().setAllowRegistration(true);
        setupInsertersList();
        createDefaultInserters();
        InserterRegistry.getInstance().setAllowRegistration(false);
    ...
    private static void setupInsertersList() {
        ...
        for (var file : files) {
            ...
            read(file, id).ifPresent(InserterRegistry.getInstance()::registerInserter);
        }
    }
    ...
    private static void createDefaultInserters() {
        InserterDefaults.all().stream()
                .filter(inserter -> EarlyConfig.shouldGenerateInserter(inserter.getName()))
                .forEach(InserterRegistry.getInstance()::registerInserter);
    }
}
